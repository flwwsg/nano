...
import (
	"fmt"
	"github.com/lonng/nano/examples/demo/chatpb/protocol"
...
func (mgr *RoomManager) Join(s *session.Session, msg []byte) error {
	...
	s.Push("onMembers", &protocol.AllMembers{Members: room.group.Members()})
	// notify others
	room.group.Broadcast("onNewUser", &protocol.NewUser{Content: fmt.Sprintf("New user: %d", s.ID())})
	// new user join group
	room.group.Add(s) // add session to group
	return s.Response(&protocol.JoinResponse{Result: "success", Code: 200})
}

// Message sync last message to all members
func (mgr *RoomManager) Message(s *session.Session, msg *protocol.UserMessage) error {
	if !s.HasKey(roomIDKey) {
